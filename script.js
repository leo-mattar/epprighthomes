...
function buttonHover() {
  $("[data-btn='wrap']").each(function () {
    ...
    $(this).on("mouseenter", function (e) {
      ...
      if ($(this).attr("btn-style") === "outline-dark" && !$(this).hasClass('schedule')) {
        gsap.to($(this), {
...
function megamenuDesktop() {
  ...
  links.forEach(link => {
    ...
    let tl = gsap.timeline({ paused: true, defaults: { ease: "expo.inOut", duration: 0.8 } });
...
function projectsSlider() {
  ...
  if (document.querySelector(".swiper.gallery-20-main") !== null) {
    ...
    let slideCount = document.querySelectorAll(".swiper-slide.gallery-20-main").length;
    let slideCountEl = document.querySelector("[gallery-20-main-length]");
    slideCountEl.textContent = slideCount;

    eventsSliderMain.on("slideChange", function (e) {
      let slideNumber = e.realIndex + 1;
      document.querySelector("[gallery-20-main-current-index]").textContent = slideNumber;
    });
  }
}
...
function ourProcessAccordion() {
  ...
  if (accordions.length > 0) {
    accordions.forEach((accordion, index) => {
      ...
      tl.to(accordionNumberCircle, {
        backgroundColor: "#E6B638",
        border: "1px solid #E6B638",
        color: "#1D1D18"
      }, 0);
      if (index > 0) {
        tl.to(previousAccordion.querySelector(".c-line"), { backgroundColor: "#53565A" }, 0);
      }
...
function homeLoader() {
  ...
  let tl = gsap.timeline({ defaults: { ease: "expo.out", duration: 1.6, delay: 0.3 } });
  ...
  if (window.matchMedia("(min-width: 992px)").matches) {
    ...
    tl.fromTo(
      heroImgWrap, {
        ...
        rotation: -
          15
      }, { clipPath: "inset(0% 0% 0% 0% round 0em)", rotation: 0 }, 0
    );
  } else {
    ...
    tl.fromTo(
      heroImgWrap, { clipPath: "inset(50% 50% 50% 50% round 1em)" }, { clipPath: "inset(0% 0% 0% 0% round 0em)" },
      0
    );
  }
